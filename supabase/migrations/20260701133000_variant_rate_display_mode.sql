...
begin
  if not exists (
    ...
    where type.typname = 'variant_rate_display_mode'
      and namespace.nspname = 'public'
  ) then
    create type public.variant_rate_display_mode as enum ('fixed', 'on_call');
  end if;
end;
$$;

alter table public.product_variants
add column if not exists rate_display_mode public.variant_rate_display_mode not null default 'fixed';
...
begin
  ...
  select
    ...
    coalesce(
      sum(
        case
          when pv.rate_display_mode = 'fixed' then ci.quantity * pv.price_inr
          else 0
        end
      ),
      0
  ...
  select
    ...
    pv.unit,
    pv.unit_value,
    case when pv.rate_display_mode = 'fixed' then pv.price_inr else 0 end,
    ci.quantity,
    case when pv.rate_display_mode = 'fixed' then ci.quantity * pv.price_inr else 0 end,
    pv.rate_display_mode
  from checkout_rpc_items ci
  join public.product_variants pv on pv.id = ci.variant_id
  ...
  select jsonb_agg(
    jsonb_build_object(
      ...
      'rateDisplayMode', oi.rate_display_mode
    )
